...
from src.schemas import (
    UserReadSchema, SubjectReadSchema, AttachmentReadSchema,
    TaskChoiceReadSchema, TaskTextReadSchema, TaskFileReadSchema,
)
...
async def subjects_get(request: Request, user: UserReadSchema = Depends(get_current_user)):
    ...
    for subject in all_subjects:
        subject.can_edit = subject.id in redactor_ids
...
async def edit_subject(
    ...
    if not (user.is_teacher or is_subject_redactor):
...
async def topics_get(request: Request, subject_id: int, user: UserReadSchema = Depends(get_current_user)):
    subject = await db.get_subject_by_id(subject_id)

    if subject:
        topics = await db.get_topics_by_subject_id(subject_id, user.id, user.is_teacher)
...
async def edit_topic(
    ...
    if not (user.is_teacher or is_subject_redactor or is_topic_redactor):
...
async def tasks_list(
    ...
    if topic and subject:
        ...
        tasks = await db.get_tasks_by_topic_id(topic_id, user_id=user.id, is_teacher=user.is_teacher)

        if tasks is None:
            tasks = []

        attempts = {}
        if not user.is_teacher and tasks:
...
async def create_task(
    ...
    if task_attachments and task_attachments[0].filename:
        upload_dir = UPLOADS_DIR / "tasks" / str(task_id)
...
async def edit_task(
    ...
    if not (user.is_teacher or is_subject_redactor or is_topic_redactor or is_task_redactor):
...
async def check_answer_endpoint(
    ...
    if isinstance(task, TaskChoiceReadSchema):
    ...
    elif isinstance(task, TaskTextReadSchema):
        user_answer = form_data.get('user_answer', '').strip()
        correct_answer = task.correct_answer.strip()
        is_case_sensitive = task.is_case_sensitive
    ...
    elif isinstance(task, TaskFileReadSchema) and task.question_type == 'material':
        return JSONResponse(status_code=400, content={
            "is_correct": False,
            "message": "Конспект/материал — не требует ответа."
        })

    elif isinstance(task, TaskFileReadSchema):
        ...
        upload_dir = UPLOADS_DIR / "student_answers" / str(user.id) / str(task_id)
        makedirs(upload_dir, exist_ok=True)
...
async def get_subject_redactors_list(subject_id: int, user: UserReadSchema = Depends(get_current_user)):
    can_edit = await db.is_user_subject_redactor(subject_id, user.id)
    if not can_edit:
    ...
    return [
        {**r.model_dump(), 'protected': (r.id == creator_id)}
        for r in redactors
    ]


@subjects_route.get('/subjects/{subject_id}/students')
async def get_subject_students_list(subject_id: int, user: UserReadSchema = Depends(get_current_user)):
    can_edit = await db.is_user_subject_redactor(subject_id, user.id)
    if not can_edit:
        raise HTTPException(status_code=403)
...
async def get_topic_redactors_list(subject_id: int, topic_id: int, user: UserReadSchema = Depends(get_current_user)):
    can_edit = await db.is_user_topic_redactor(topic_id, user.id)
    if not can_edit:
        raise HTTPException(status_code=403)
    ...
    return [
        {**r.model_dump(), 'protected': (r.id == topic_creator or r.id == subject_creator)}
        for r in redactors
    ]


@subjects_route.get('/subjects/{subject_id}/{topic_id}/students')
async def get_topic_students_list(subject_id: int, topic_id: int, user: UserReadSchema = Depends(get_current_user)):
    can_edit = await db.is_user_topic_redactor(topic_id, user.id)
    if not can_edit:
        raise HTTPException(status_code=403)
...
async def get_task_redactors_list(subject_id: int, topic_id: int, task_id: int, user: UserReadSchema = Depends(get_current_user)):
    can_edit = await db.is_user_task_redactor(task_id, user.id)
    if not can_edit:
        raise HTTPException(status_code=403)
    task = await db.get_task_by_id(task_id)
    task_creator = task.created_by if task else None
    topic_creator = await db.get_topic_creator(topic_id)
    subject_creator = await db.get_subject_creator(subject_id)
    redactors = await db.get_task_redactors(task_id)
    return [
        {**r.model_dump(), 'protected': (r.id == task_creator or r.id == topic_creator or r.id == subject_creator)}
        for r in redactors
    ]


@subjects_route.get('/subjects/{subject_id}/{topic_id}/{task_id}/students')
async def get_task_students_list(subject_id: int, topic_id: int, task_id: int, user: UserReadSchema = Depends(get_current_user)):
    can_edit = await db.is_user_task_redactor(task_id, user.id)
    if not can_edit:
        raise HTTPException(status_code=403)
...
async def get_task_submissions_list(subject_id: int, topic_id: int, task_id: int, user: UserReadSchema = Depends(get_current_user)):
    can_edit = await db.is_user_task_redactor(task_id, user.id)
    if not can_edit:
        raise HTTPException(status_code=403)
